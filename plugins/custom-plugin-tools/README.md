...
| `agent-test-planner` | `## Test plan` | Reads changed skills/agents/ and produces a specific, invocation-level test checklist |
| `agent-sdet` | _(no PR section)_ | Finds gaps in `tests/**/` for changed scripts and writes missing `test-*.sh` files |
